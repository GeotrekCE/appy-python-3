...
class Updater:
    ...
    def __init__(self, trFolder):
        ...
        self.config = importModule('app', str(appPath)).Config
    ...
    def visitGroup(self, class_, group, walkedGroups):
        ...
        # Do nothing if p_group is None, if a label must not be generated for
        # it, or if labels have already been generated for him.
        if not group or group.label or (group in walkedGroups): return
    ...
    def visitPhase(self, class_, phase):
        ...
        # Produce one label for every page, excepted if there is a single page
        if len(phase.pages) == 1: return
    ...
    def visitWorkflow(self, name, workflow):
        ...
        for attribute in workflow.attributes:
            for element in getattr(workflow, attribute).values():
                ...
                if (attribute == 'transition') and element.confirm:
    ...
    def collectLabels(self):
        ...
        for role in model.getRoles(base=True):
