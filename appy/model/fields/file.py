...
import os, os.path, time, shutil, base64
...
class FileInfo:
    ...
    def __init__(self, fsPath, inDb=True, uploadName=None):
        ...
        if not inDb:
            ...
            self.mimeType = putils.guessMimeType(self.fsPath)
    ...
    def getFilePath(self, obj):
        ...
        r = putils.osPathJoin(dbFolder, folder, self.fsName)
        if not os.path.exists(r):
            ...
            r = putils.osPathJoin(sutils.getOsTempFolder(), folder, self.fsName)
        return r

    def removeFile(self, dbFolder='', removeEmptyFolders=False):
        '''Removes the file from the filesystem.'''
        try:
            os.remove(putils.osPathJoin(dbFolder, self.fsPath, self.fsName))
        except Exception:
        ...
        if removeEmptyFolders:
            path = putils.osPathJoin(dbFolder,self.fsPath)
            putils.FolderDeleter.deleteEmpty(path)
    ...
    def writeFile(self, fieldName, fileObj, dbFolder):
        ...
        fsName = putils.osPathJoin(dbFolder, self.fsPath, self.fsName)
        f = open(fsName, 'wb')
        if fileType == 'FileUpload':
...
class File(Field):
    ...
    edit = Px('''
     <x var="fName='%s_file' % name">
      <x if="value">:field.view</x><br if="value"/>
      <x if="value">
    ...
    def __init__(self, validator=None, multiplicity=(0,1), default=None,
      ...
      label=None, isImage=False, downloadAction=None, sdefault='', scolspan=1,
      swidth=None, sheight=None, view=None, cell=None, edit=None, xml=None,
      translations=None, render=None, icon='paperclip', nameStorer=None):
        ...
        # Call the base constructor. Regarding attribute "width":
        # ----------------------------------------------------------------------
        #  if used for... | it represents...
        # ----------------------------------------------------------------------
        #    a file       | the width of the input field allowing to upload the
        #                 | file;
        #   an image      | the maximum width for rendering the image in the
        #                 | web UI. It will not resize the uploaded image.
        # ----------------------------------------------------------------------
    ...
    def getDownloadLink(self, name, obj, layoutType):
        ...
        if self.isImage:
            ...
            css = ' style="max-width: %s"' % self.width if self.width else ''
    ...
    def store(self, obj, value):
        ...
        if value:
            ...
            if isinstance(value, cfg.FileUpload) or isinstance(value, cfg.File):
                ...
                mimeType = mimeType or putils.guessMimeType(fileName)
