...
HELP_FORCE_LOAD = 'LibreOffice, in server mode as well as used from the UI, ' \
  'may take some time to load a (large) document in its entirety. If you set ' \
  'this parameter, the converter will send him commands like "jump to the ' \
  'last page" to force him to load the document in it entirety, before doing ' \
  'other things like updating the table of contents.'
...
class Converter:
    ...
    def __init__(self, docPath, result, server=DEFAULT_SERVER,
                 ...
                 pdfOptions=None, ppp=False, stream='auto', forceLoad=False,
                 pageStart=1, verbose=False):
        ...
        # on disk ?
        self.stream = stream
        # Force LO to load the document in its entirety ?
    ...
    def updateOdtDocument(self):
        ...
        # Force LO to read to document in its entirety
        if self.forceLoad:
...
class ConverterScript:
    ...
    def run(self):
        ...
        add('-l', '--forceLoad', action='store_true', help=HELP_FORCE_LOAD)
        ...
        converter = Converter(args[0], args[1], options.server, options.port,
          ...
          options.pdf, options.ppp, stream, options.forceLoad,
          options.pageStart, options.verbose)
        try:
            converter.run()
